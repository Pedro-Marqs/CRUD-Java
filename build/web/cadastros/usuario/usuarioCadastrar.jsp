...
<script>
    $(document).ready(function () {
        ...
        //EMAIL no console
        $('#email').blur(function () {
            console.log("verificando email no backend");
            // Se passou pela validação local, faz a verificação no backend
            if ($('#email').val() == email) {
                $.ajax({
                    type: 'get',
                    url: 'UsuarioVerificarEmail', // sua servlet ou endpoint no backend
                    data: {email: email},
                    success: function (response) {
                        console.log("resposta validacao backend:")
                        console.log(response);
                        if (response == '1') {
                            Swal.fire({
                                ...
                                text: 'Por favor, verifique o email informado.',
                                showConfirmButton: true,
                                timer: 4000
                            }).then(function () {
                                $('#nome').focus();
                    ...
                    error: function () {
                        console.log("Erro ao verificar email no servidor.");
                    }
                });
            }
        });
